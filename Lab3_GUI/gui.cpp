...
gui_l3::gui_l3(QWidget *parent) : gui(parent) {
	...
	connect(m_b, &QPushButton::clicked, [this]() {
		...
		//auto max_state = pn::PetriNetSystemSimulator<size_t>::simulate(m_canvas->nets(), tasks, ui.total_tasks->value() * m_canvas->nets().size() * m_canvas->nets().size());
		...
		//		}
		//}
		//results.push_back(std::make_pair("NA_Time_persent", max_state));		
